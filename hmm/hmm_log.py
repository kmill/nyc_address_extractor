...
class HMM(object) :
    ...
    def initialize(self, states=None, alphabet=None, p_init=None, p_trans=None, p_emit=None) :
        ...
        self.pi = { k : log(v) for (k,v) in p_init.iteritems() }
        self.A = { k1 : { k2 : log(v) for (k2, v) in p_trans[k1].iteritems() } for k1 in p_trans }
        self.B = { k1 : { k2 : log(v) for (k2, v) in p_emit[k1].iteritems() } for k1 in p_emit }
    ...
    def maximization(self, obs) :
        self.pi = {i : log(sum(exp(self.P[k] + self.gamma[k][0][i]) for k in xrange(len(obs))))
                       - log(sum(exp(self.P[k]) for k in xrange(len(obs))))
                   for i in self.states}

        self.A = {i : {j : log(sum(exp(self.P[k]) * sum(exp(self.xi[k][t][(i,j)]) for t in xrange(len(obs[k]) - 1))
                                   for k in xrange(len(obs))))
                           - log(sum(exp(self.P[k]) * sum(exp(self.gamma[k][t][i]) for t in xrange(len(obs[k]) - 1))
                                     for k in xrange(len(obs))))
                       for j in self.states}
                  for i in self.states}

        self.B = {i : { v : log(sum(exp(self.P[k]) * sum(exp(self.gamma[k][t][i] if obs[k][t] == v else 0)
                                                      for t in xrange(len(obs[k])-1))
                                for k in xrange(len(obs))))
                            - log(sum(exp(self.P[k]) * sum(exp(self.gamma[k][t][i])
                                                        for t in xrange(len(obs[k])-1))
                                  for k in xrange(len(obs))))
    ...
    def calculate_xi_i_j_t(self, k, i, j, t, obs) :
        ...
        denominator = log(sum(exp(self.alpha[k][t][i_d] \
                                  + self.A[i_d][j_d] \
                                  + self.B[j_d][obs_t1] \
                                  + self.beta[k][t+1][j_d])
    ...
    def calculate_gamma(self, obs) :
        self.gamma = [[{i : log(sum(exp(self.xi[k][t][(i,j)])
                                    for j in self.states))
                        for i in self.states}
                       for t in xrange(len(obs[k]) - 1)]
                      for k in xrange(len(obs))]
    def calculate_P(self, obs) :
        self.P = [log(sum(exp(self.alpha[k][-1][i]) for i in self.states)) for k in xrange(len(obs))]
    #forward procedure
    def calculate_alpha(self, obs) :
        # 1) Initialization
        self.alpha = [[ { i : self.pi[i] + self.B[i][obs[k][0]] for i in self.states} ]
                      for k in xrange(len(obs)) ]
        # 2) Induction
        for k in xrange(len(obs)) :
            for t in xrange(len(obs[k]) - 1) :
#                print { j : (self.B[j][obs[k][t+1]],
#                             log(sum(exp(self.alpha[k][t][i] + self.A[i][j])
#                                     for i in self.states)))
#                        for j in self.states }
                self.alpha[k].append({ j : self.B[j][obs[k][t+1]] \
                                           + log(sum(exp(self.alpha[k][t][i] + self.A[i][j])
                                                     for i in self.states))
                                       for j in self.states })
        # after first calculation, alpha is in ln
        # assume pi, B, A, alpha are in ln
        # TODO: initialize A,B,pi with log likelihood
    #backward procedure
    def calculate_beta(self, obs) :
        ...
        # 2) Induction
        for k in xrange(len(obs)) : 
            self.beta[k][-1] = { i : log(1.0) for i in self.states }
            for t in xrange(2, len(obs[k])+1) :
                self.beta[k][-t] = { i : log(sum(exp(self.A[i][j] + self.B[j][obs[k][1-t]] + self.beta[k][1-t][j])
                                             for j in self.states))
                                     for i in self.states }
